...
src/
  ...
  content/devlog/<slug>/  папка на запись: index.md + её картинки
...
Запись — папка `src/content/devlog/<slug>/` с `index.md` внутри; имя папки
становится адресом, картинки записи лежат там же.
...
ссылки на него. Для агента в проекте клиента — `docs/devlog-handoff.md`:
что собирать по ходу работы, чтобы из этого вышла запись.
...
#### Как человек попадает на свой язык

Никакого определения страны по IP: сайт статический, сервера, который видел бы
IP, нет, и узнать страну можно только сходив в чужой геосервис — то есть
нарушив «ни одного стороннего запроса». Да и язык по стране угадывается плохо:
VPN, поездка, эмигрант.

Вместо этого `src/components/LangHint.astro` смотрит `navigator.languages` —
браузер уже знает, на каких языках человек читает, потому что человек сам ему
это сказал. Если подходящий язык у нас есть, а страница открыта не на нём,
над шапкой появляется одна строка **на предлагаемом языке** со ссылкой на ту
же страницу.

Именно подсказка, а не редирект: молчаливый редирект ломает кнопку «назад»,
открывает пересланную ссылку не тем, чем её отправляли, и отдаёт роботу не то,
что обещано в `hreflang`.

Выбор запоминается в `localStorage` под ключом из `src/lib/lang-pref.ts`.
Пишут туда два места: переключатель языка (человек выбрал явно) и крестик на
подсказке (текущий язык устраивает). После любого из них подсказка молчит.
Это настройка, а не слежка — код языка, который никуда не уходит.

Разметка всех вариантов отрисована заранее и скрыта, скрипт лишь снимает
`hidden`. Скрытый текст не тянет шрифты — подмножества по `unicode-range`
качаются только под то, что реально рисуется, — а инлайновый скрипт сразу за
разметкой успевает отработать до первой отрисовки, поэтому полоса не
доскакивает и не сдвигает текст.
